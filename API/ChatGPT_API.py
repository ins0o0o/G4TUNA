...
import openai  # OpenAI 라이브러리 설치 필요: pip install openai
...
today_date_hyphen = datetime.today().strftime('%Y-%m-%d')

# Google API Key
google_key = "AIzaSyA2KUAo4fugxxjo4zG2iHMy1FS70zbls8A"

# 1. 현재 온도, 강수확률, 하늘상태 가져오기
def get_weather_forecast():
    ...
    if sky_status is not None:
        if sky_status == '1':
            print("하늘 상태: 맑은 하늘입니다.")
        elif sky_status == '2':
            print("하늘 상태: 구름이 조금 있습니다.")
        elif sky_status == '3':
            print("하늘 상태: 구름이 많습니다.")
        elif int(sky_status) >= 4:
            print("하늘 상태: 흐립니다.")
    else:
    ...
    if response.status_code == 200:
        ...
        if not events:
            ...
            for event in events:
                ...
                recommend_supplies(title)  # 준비물 추천 함수 호출
    else:
